      ...
      <div className="bg-white w-64 sm:w-full mx-auto relative rounded-md p-3 border border-purple group overflow-hidden hover:shadow-xl hover:shadow-purple/20 duration-300">
        ...
        <div className="relative w-24 mx-auto">
          ...
          {rating.length > 0 && (
            <span className="absolute bottom-1 right-1 bg-white text-[#FDCC0D] text-sm w-7 group-hover:w-12 flex items-center justify-center gap-[2px] px-1 rounded-md shadow-xl duration-300">
              {rating[0].rating}{" "}
              <BiSolidStar className="hidden group-hover:inline" />
            </span>
          )}
        </div>
        ...
        <div className="text-center mt-3">
          <Link
            ...
            className="text-dark text-3xl font-medium capitalize line-clamp-1"
          >
            {name}
          </Link>

          <h3 className="text-lightGray text-lg">{category}</h3>
          <p className="text-gray text-sm flex items-center justify-center italic">
            <BiMap /> {location}
          </p>
        ...
        <Link
          ...
          to={`/candidate_details/${_id}`}
          onClick={() => viewsCount()}
          className="bg-purple text-white inline-block p-3 rounded-md absolute top-1 right-1 md:-right-16 group-hover:right-1 duration-300 hover:bg-dark shadow-xl shadow-purple/20 hover:shadow-dark/20"
        >
