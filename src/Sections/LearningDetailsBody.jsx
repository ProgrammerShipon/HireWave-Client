...
const LearningDetailsBody = () => {
  ...
  const { title, createdAt, videoLink, description, authorImg, authorName, authorEmail, comments, disLike, like, views, _id } = loadData;
            ...
            <div >
              {
                seeAll ?
                  ...
                  <> <p
                    className="text-lightGray mt-4"
                    onClick={() => { setSeeAll(!seeAll) }}
                    dangerouslySetInnerHTML={{ __html: description.slice(0, 320) }}
                  ></p></>
          ...
          <div className='hidden lg:block col-span-1'>
            {
              sidebarContent.map((sidebar , index) =>
                <div key={index} className="p-2 rounded-lg hover:shadow-xl hover:shadow-green/20 border border-dark/40 hover:border-green duration-300 mb-3">
